...
export function getOperatorRows(graph: { operators: GraphOperator[] }): Record<string, unknown>[] {
  const ops = graph.operators.filter((n: GraphOperator) => !n.metadata?.isTensorNode);
  const allKeys = new Set<string>(['index', 'id', 'name']);

  // 收集所有可能的键：从 metadata 和 attributes
  ops.forEach((n: GraphOperator) => {
